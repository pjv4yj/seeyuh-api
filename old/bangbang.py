...
async def create_super_agent() -> Agent:
    ...
    super_agent = Agent(
        ...
        instructions=(
            ...
            f"**Enhanced Workflow for DEEP comprehensive analysis:**\n"
            f"1. **EXTENSIVE Ghidra Analysis**: Hand off to Ghidra Binary Analyst for thorough reverse engineering (10-15 functions minimum)\n"
            f"2. **COMPREHENSIVE Intelligence**: Hand off to Known Vulnerability Researcher for extensive CVE research (5-8 searches minimum)\n"
            f"3. **DEEP Pattern Analysis**: Hand off to Vulnerability Reasoning Expert for detailed vulnerability assessment\n"
            f"4. **CONTINUE ANALYSIS**: Keep handing off until you have comprehensive findings\n"
            f"5. **SYNTHESIS**: Create detailed final assessment with actionable intelligence\n\n"
            f"**Focus Areas:**\n"
...
async def main():
    ...
    queries = [
        ...
        f"Perform EXTENSIVE analysis of {BINARY_PATH} (already loaded in Ghidra) for zero-click vulnerabilities. Conduct deep Ghidra function analysis (10+ functions), comprehensive web research (5+ searches), and thorough vulnerability reasoning. Use dynamic shell commands based on findings. Focus on memory corruption bugs, parsing functions, and exploitation paths. Provide detailed technical analysis with specific functions, addresses, CVEs, and actionable intelligence.",
